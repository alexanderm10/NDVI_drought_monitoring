...
# MODIFIED VERSION: Downloads only B04, B05, B8A, and Fmask bands
# Original script: https://github.com/nasa/HLS-Data-Resources/tree/main/bash/hls-bulk-download
# Modified by: Claude Code for NDVI drought monitoring project
# Modification date: 2026-02-03
#
# CHANGES FROM ORIGINAL:
#   Line 237: Added regex filter to download only specific bands:
#     - B04 (Red) - both L30 and S30
#     - B05 (NIR) - L30 only
#     - B8A (NIR narrow) - S30 only
#     - Fmask (Quality) - both L30 and S30
#   This reduces download size by ~60-70% compared to full granules
#
...
#       The default is 10; can be modified based on the capacity of the local computer.
...
# 	They appear to have the same speed.
...
function download_granule()
{
	...
	# MODIFIED: Filter to only download specific bands (B04, B05, B8A, Fmask)
	# Original line: grep $granule $flist > $allfile
