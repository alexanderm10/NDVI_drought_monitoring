...
config <- list(
  ...
  # Checkpointing (increased interval to reduce overhead with large checkpoints)
...
calculate_baseline_derivatives <- function(timeseries_df, config) {
  ...
  for (i in seq_along(pixel_batches)) {
    ...
    if (n_processed_this_run > 0 && (n_processed_this_run - last_saved_checkpoint) >= config$checkpoint_interval) {
      ...
      derivatives_df <- rbind(derivatives_df, new_data)
  ...
  # If all_results is empty, derivatives_df already has all data from last checkpoint
