# ==============================================================================
# 03_test_k50_year_predictions.R
#
# Purpose: TEST k=50 spatial basis on 4 test years
# Based on: 03_doy_looped_year_predictions.R
#
# Approach:
...
config <- list(
  ...
  # Testing k=50 as middle ground between k=30 (stable) and k=80 (overfitting)
  # k=80 had 207K negative predictions - too high for data density
  # k=50 should provide improved resolution while staying stable
  spatial_k = 50,

  # Output (year-by-year files) - k=50 test directories
  output_dir = file.path(hls_paths$gam_models, "modeled_ndvi_k50_test"),
  stats_file = file.path(hls_paths$gam_models, "modeled_ndvi_k50_test_stats.rds"),

  # Posteriors directory (organized by year, incremental saving to avoid memory issues)
  posteriors_dir = file.path(hls_paths$gam_models, "year_predictions_posteriors_k50_test"),
