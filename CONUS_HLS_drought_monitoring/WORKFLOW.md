...
- `ensemble_or` — Logical-OR ensemble of `ndvi_z` + `spei_13w` at headline op (2026-06-17). Tests whether complementarity translates to operational lift. Per-event hit rate AND temporal-block HSS for {ndvi, spei, or}. Output: `ensemble_or_{scope}.rds` (~160 MB). Runtime ~10 min. **Finding: OR lifts hit rate +14-20 pts but HSS roughly cancels (POD up, FAR up).** Note: superseded by `ensemble_multi` which shows `spei_4w` is the better SPEI partner than `spei_13w`.
- `ensemble_multi` — Broader ensemble sweep: Tier 1 (8 single signals) + Tier 3 (3 cross-pairs: ndvi_z + spei_{4w,13w,26w}) × z-sweep (1.0/1.5/2.0) × 2 directions. Per-event hit rate + block-based HSS per cell. Output: `ensemble_multi_{scope}.rds` (~160 KB long format). Runtime ~62 min. **Finding: `ndvi_z OR spei_4w` is the best cross-pair at every z; +15.5 pt onset / +13.9 pt recovery lift over best single signal (spei_4w). HSS lift is ≤0 domain-wide for all pairs (POD up, FAR up), but ~1/3 of strata DO show positive per-cell HSS lift.**
- `qc` — stub
